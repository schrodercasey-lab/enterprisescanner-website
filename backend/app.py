...
from api.threat_intelligence import threat_intel_bp
from api.user_management import user_mgmt_bp
from api.api_security import api_security_bp
...
# Register enterprise chat blueprint
try:
    from api.enterprise_chat import enterprise_chat_bp
    app.register_blueprint(enterprise_chat_bp, url_prefix='/api')
    print("Enterprise chat API registered successfully")
except ImportError as e:
    print(f"Warning: Could not import enterprise chat API: {e}")
...
def demo_request():
    ...
    crm_entry = {
        ...
        'interest': data.get('interest', 'Demo Request'),
        'source': 'Website Demo Request',
        'lead_score': calculate_lead_score(data),
        'is_fortune_500': is_fortune_500_company(data.get('company', '')),
        'status': 'New Lead - Demo Requested'
    }
    
    # Send to sales team via email
    try:
        send_crm_notification(crm_entry, recipient='sales@enterprisescanner.com')
        print(f"✅ CRM notification sent to sales team")
    except Exception as e:
        print(f"❌ Failed to send CRM notification: {e}")
    
    # Schedule automated follow-up
    schedule_follow_up_emails(crm_entry)
...
def security_assessment():
    ...
    employee_count = form_data.get('employeeCount', 'Unknown')
    ...
    crm_entry = {
        ...
        'industry': industry,
        'annual_revenue': form_data.get('revenue', 'Unknown'),
        'employee_count': employee_count,
        'risk_score': risk_score,
        'business_value': business_value,
        'is_high_value': is_high_value,
        'is_fortune_500': is_fortune_500_company(form_data.get('company', '')),
        'source': 'Security Assessment Portal',
        'lead_score': calculate_lead_score(form_data) + (20 if is_high_value else 0),
        'status': 'Assessment Completed - Follow-up Required',
        'next_action': 'Sales Contact',
        'follow_up_actions': follow_up_actions
    }
    
    # Send assessment results to sales@enterprisescanner.com
    try:
        send_crm_notification(crm_entry, recipient='sales@enterprisescanner.com')
        print(f"✅ Assessment results sent to sales@enterprisescanner.com")
    except Exception as e:
        print(f"❌ Failed to send to sales team: {e}")
    
    # Schedule automated follow-up emails
    follow_up_schedule = [
        {'delay_hours': 24, 'template': 'assessment_results_followup', 'priority': 'high' if is_high_value else 'normal'},
        {'delay_hours': 72, 'template': 'consultation_offer', 'priority': 'high' if is_high_value else 'normal'},
        {'delay_hours': 168, 'template': 'case_study_share', 'priority': 'normal'}  # 1 week
    ]
    ...
    # Add to Fortune 500 prospect pipeline if applicable
    if is_fortune_500_company(form_data.get('company', '')) or is_high_value:
        try:
            add_to_fortune_500_pipeline(crm_entry)
            print(f"✅ Added to Fortune 500 prospect pipeline")
            
            # Immediate high-priority notification to sales leadership
            send_crm_notification(
                crm_entry, 
                recipient='sales@enterprisescanner.com',
                subject=f'🎯 FORTUNE 500 LEAD: {form_data.get("company")} - Risk Score {risk_score}/100',
                priority='urgent'
            )
        except Exception as e:
            print(f"❌ Failed to add to Fortune 500 pipeline: {e}")
...
# SocketIO Event Handlers for Enterprise Chat
@socketio.on('connect')
def handle_connect():
    """Handle client connection to chat"""
    print(f"Client connected: {request.sid}")
    emit('connected', {'status': 'success', 'message': 'Connected to Enterprise Scanner chat'})
...
def handle_join_chat(data):
    ...
    if chat_id:
        ...
        join_room(chat_id)
        emit('joined_chat', {'chat_id': chat_id, 'message': 'Joined chat successfully'})
...
def handle_leave_chat(data):
    ...
    if chat_id:
        ...
        leave_room(chat_id)
        emit('left_chat', {'chat_id': chat_id, 'message': 'Left chat successfully'})
...
def handle_send_message(data):
    ...
    try:
        # Import chat manager
        from services.enterprise_chat import EnterpriseChatManager
        ...
        if not chat_id or not message_content:
            emit('error', {'message': 'Missing chat_id or message content'})
            return
        ...
        # Emit response to the chat room
...
def handle_typing_start(data):
    ...
    chat_id = data.get('chat_id')
    if chat_id:
        emit('user_typing', {'typing': True, 'chat_id': chat_id}, room=chat_id, include_self=False)
...
def handle_typing_stop(data):
    ...
    chat_id = data.get('chat_id')
    if chat_id:
        emit('user_typing', {'typing': False, 'chat_id': chat_id}, room=chat_id, include_self=False)

# ============================================================================
# CRM Integration Helper Functions
# ============================================================================

def send_crm_notification(crm_entry: dict, recipient: str, subject: str = None, priority: str = 'normal'):
    """Send CRM notification email to sales/support team"""
    import smtplib
    from email.mime.text import MIMEText
    from email.mime.multipart import MIMEMultipart
    ...
    try:
        # Default subject
        if not subject:
            subject = f"New Lead: {crm_entry.get('company', 'Unknown Company')}"
        
        # Create email message
        msg = MIMEMultipart('alternative')
        msg['Subject'] = subject
        msg['From'] = 'info@enterprisescanner.com'
        msg['To'] = recipient
        ...
        <body style="font-family: Arial, sans-serif;">
            <h2 style="color: #1a73e8;">New Lead Notification</h2>
            
            <table style="border-collapse: collapse; width: 100%; max-width: 600px;">
                <tr style="background-color: #f0f0f0;">
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Company:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('company', 'N/A')}</td>
                </tr>
                <tr>
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Contact:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('contact_name', 'N/A')}</td>
                </tr>
                <tr style="background-color: #f0f0f0;">
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Email:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('email', 'N/A')}</td>
                </tr>
                <tr>
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Phone:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('phone', 'N/A')}</td>
                </tr>
                <tr style="background-color: #f0f0f0;">
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Title:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('title', 'N/A')}</td>
                </tr>
                <tr>
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Industry:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('industry', 'N/A')}</td>
                </tr>
                <tr style="background-color: #f0f0f0;">
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Lead Score:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('lead_score', 0)}/100</td>
                </tr>
                <tr>
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Fortune 500:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{'✅ YES' if crm_entry.get('is_fortune_500') else '❌ NO'}</td>
                </tr>
                <tr style="background-color: #f0f0f0;">
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Status:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('status', 'N/A')}</td>
                </tr>
                <tr>
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Source:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('source', 'N/A')}</td>
                </tr>
                <tr style="background-color: #f0f0f0;">
                    <td style="padding: 10px; border: 1px solid #ddd;"><strong>Timestamp:</strong></td>
                    <td style="padding: 10px; border: 1px solid #ddd;">{crm_entry.get('timestamp', 'N/A')}</td>
                </tr>
            </table>
            
            <p style="margin-top: 20px; color: #666;">
                This is an automated notification from Enterprise Scanner CRM system.
            </p>
        </body>
        </html>
        """
        ...
        # Send via SMTP (configure with actual SMTP settings)
        # For now, just log the email
        print(f"📧 CRM Email would be sent to {recipient}:")
        print(f"   Subject: {subject}")
        print(f"   Priority: {priority}")
        print(f"   Company: {crm_entry.get('company')}")
        
        # TODO: Uncomment when SMTP is configured
        # smtp = smtplib.SMTP('smtp.gmail.com', 587)
        # smtp.starttls()
        # smtp.login('info@enterprisescanner.com', 'password')
...
def schedule_follow_up_email(recipient: str, template: str, delay_hours: int, context: dict, priority: str = 'normal'):
    ...
    # In production, this would be stored in database and processed by background worker
    print(f"📅 Follow-up email scheduled:")
    print(f"   To: {recipient}")
    print(f"   Template: {template}")
    print(f"   Scheduled: {scheduled_time} ({delay_hours}h from now)")
    print(f"   Priority: {priority}")
    
    # TODO: Store in database and use Celery/background task queue
    # db.follow_up_emails.insert(follow_up_data)
    
    return follow_up_data

def schedule_follow_up_emails(crm_entry: dict):
    """Schedule multiple follow-up emails based on lead type"""
    email = crm_entry.get('email')
    company = crm_entry.get('company')
    is_f500 = crm_entry.get('is_fortune_500', False)
    
    # Different schedules for Fortune 500 vs regular leads
    if is_f500:
        schedule = [
            {'delay_hours': 2, 'template': 'immediate_demo_offer', 'priority': 'urgent'},
            {'delay_hours': 24, 'template': 'executive_case_study', 'priority': 'high'},
            {'delay_hours': 48, 'template': 'personalized_roi_analysis', 'priority': 'high'}
        ]
    else:
        schedule = [
            {'delay_hours': 24, 'template': 'demo_confirmation', 'priority': 'normal'},
            {'delay_hours': 72, 'template': 'feature_highlights', 'priority': 'normal'},
            {'delay_hours': 168, 'template': 'success_stories', 'priority': 'normal'}
        ]
    
    for follow_up in schedule:
        schedule_follow_up_email(
            recipient=email,
            template=follow_up['template'],
            delay_hours=follow_up['delay_hours'],
            context={'company': company, 'crm_entry': crm_entry},
            priority=follow_up['priority']
        )
    
    print(f"✅ Scheduled {len(schedule)} follow-up emails for {company}")

def add_to_fortune_500_pipeline(crm_entry: dict):
    """Add lead to Fortune 500 prospect pipeline"""
    pipeline_entry = {
        'timestamp': crm_entry.get('timestamp'),
        'company': crm_entry.get('company'),
        'contact_name': crm_entry.get('contact_name'),
        'email': crm_entry.get('email'),
        'phone': crm_entry.get('phone'),
        'title': crm_entry.get('title'),
        'industry': crm_entry.get('industry'),
        'lead_score': crm_entry.get('lead_score', 0),
        'risk_score': crm_entry.get('risk_score'),
        'business_value': crm_entry.get('business_value'),
        'stage': 'Discovery',
        'probability': 25,  # Initial 25% close probability
        'expected_revenue': calculate_expected_revenue(crm_entry),
        'assigned_to': 'enterprise_sales_team',
        'priority': 'HIGH',
        'next_action': 'Executive Outreach',
        'next_action_due': (datetime.utcnow() + timedelta(hours=24)).isoformat()
    }
    
    # In production, store in CRM database
    print(f"🎯 Added to Fortune 500 Pipeline:")
    print(f"   Company: {pipeline_entry['company']}")
    print(f"   Expected Revenue: ${pipeline_entry['expected_revenue']:,}")
    print(f"   Stage: {pipeline_entry['stage']}")
    print(f"   Next Action: {pipeline_entry['next_action']}")
    
    # TODO: Store in database
    # db.fortune_500_pipeline.insert(pipeline_entry)
    
    return pipeline_entry

def calculate_expected_revenue(crm_entry: dict):
    """Calculate expected revenue for a lead"""
    base_revenue = 50000  # Base annual contract value
    
    # Multipliers based on company characteristics
    multiplier = 1.0
    
    # Fortune 500 multiplier
    if crm_entry.get('is_fortune_500'):
        multiplier *= 10  # Fortune 500 = $500k average
    
    # Risk score multiplier (higher risk = higher need = higher value)
    risk_score = crm_entry.get('risk_score', 50)
    if risk_score >= 80:
        multiplier *= 2
    elif risk_score >= 60:
        multiplier *= 1.5
    
    # High value lead multiplier
    if crm_entry.get('is_high_value'):
        multiplier *= 1.5
...
if __name__ == '__main__':
    ...
    # Use SocketIO run method instead of app.run for WebSocket support
